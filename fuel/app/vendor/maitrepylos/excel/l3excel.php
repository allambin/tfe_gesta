...
class L3excel
{
    ...
    public static function excel($data,$nom_groupe)
    {
        ...
        for ($i=0;$i<24;$i++) {
            ...
            $sheet->setCellValueByColumnAndRow(0, $cellule,$data['annexe1'][1][$i]['t_nom']
                .'-'.$data['annexe1'][1][$i]['cedefop']);
            if($data['annexe1'][1][$i]['resultat']==0) {
        ...
        foreach ($nom_groupe as $value) {
            ...
            $sheet_array1[$value] = $workbook->createSheet();
            //nom de la feuille
            $sheet_array1[$value]->setTitle('Annexe 2 '.$value);
            \Maitrepylos\Excel\Annexe::excel($sheet_array1[$value],$data['xml'],$data[$value],$cedefop,$value,$data['annee']);
        ...
        foreach ($nom_groupe as $value) {
            ...
            //trouver  le cedefop
            $cedefop = \Maitrepylos\Utils::cedefop($data['cedefop'], $value);
            $sheet_array2[$value] = $workbook->createSheet();
            $sheet_array2[$value]->setTitle('Annexe 3 '.$value);
            \Maitrepylos\Excel\Stagiaire::excel($sheet_array2[$value],$cedefop,$data['participant-'.$value],
        ...
        $workbook->affiche('Excel5','Trimestre'.date('Y-m-d-H:s'));
